...
public class ConnectPlugin extends CordovaPlugin {
    ...
    private void executeGraphPost(JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        graphPath = args.getString(0);
        JSONArray arr = args.getJSONArray(1);
        final JSONObject postParams = args.getJSONObject(2);
//        final GraphObject postParams = GraphObject.Factory.create(postJSON);
    ...
    private void makeGraphPost(JSONObject postParams) {
        //If you're using the paging URLs they will be URLEncoded, let's decode them.
        try {
            graphPath = URLDecoder.decode(graphPath, "UTF-8");
        } catch (UnsupportedEncodingException e) {
        ...
        String[] urlParts = graphPath.split("\\?");
        String graphAction = urlParts[0];
        GraphRequest graphRequest = GraphRequest.newPostRequest(AccessToken.getCurrentAccessToken(), graphAction, postParams, new GraphRequest.Callback() {
            @Override
            public void onCompleted(GraphResponse response) {
                if (graphContext != null) {
                    if (response.getError() != null) {
